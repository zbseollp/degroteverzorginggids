...
(function () {
  ...
  function initMenuToggle() {
    document.querySelectorAll('.elementor-widget-nav-menu').forEach((widget) => {
      ...
      if (!toggle || !dropdown || toggle.dataset.htkInit) return;
      toggle.dataset.htkInit = '1';
  ...
  function initDesktopSubmenus() {
    document.querySelectorAll('.elementor-nav-menu--main .menu-item-has-children').forEach((item) => {
      ...
      if (!link || !submenu || item.dataset.htkDesktopInit) return;
      item.dataset.htkDesktopInit = '1';
  ...
  function initMobileSubmenus() {
    document
      ...
      .forEach((link) => {
        if (link.dataset.htkMobileInit) return;
        link.dataset.htkMobileInit = '1';
