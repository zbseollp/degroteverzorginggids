#!/usr/bin/env node
/**
 * Fetch all product guide pages from live degroteverzorginggids.nl.
 */
...
const OUT_DIR = path.join(ROOT, 'src/data/product-pages');
...
function getSlugs() {
  ...
  return data.productPages;
}

async function downloadPostCss(postCss) {
  if (!postCss) return;
  const url = `https://degroteverzorginggids.nl/wp-content/uploads/elementor/css/${postCss}`;
  try {
    await downloadCss(ROOT, url.split('?')[0]);
  } catch (err) {
    console.warn(`  Post CSS skip: ${postCss} (${err.message})`);
  }
}
...
async function main() {
  ...
  for (const slug of slugs) {
    try {
      ...
      await downloadPostCss(meta.postCss);

      for (const url of extractStylesheets(html).filter((u) => u.includes('/uploads/elementor/css/post-'))) {
        try {
          await downloadCss(ROOT, url.split('?')[0]);
        } catch {
          /* optional page CSS */
